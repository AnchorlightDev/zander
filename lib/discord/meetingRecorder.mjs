...
import { writeSilence } from "../meetings/silence.mjs";
import {
  addRecording,
...
class MeetingRecorder {
   ...
   */
  async padTrackTo(track, targetLocalOffsetMs) {
    const padded = paddingBytesFor(track.bytesWritten, targetLocalOffsetMs);
    if (padded <= 0) return 0;

    // Counted up front, so a burst that arrives while this is still draining
    // still computes its own padding against the final position rather than a
    // half-written one.
    track.bytesWritten += padded;

    // Backpressure is honoured inside writeSilence: a five minute pause is
    // 57 MB of zeroes, and queueing that whole would defeat the point of
    // streaming the recording to disk in the first place.
    await writeSilence(track.stream, padded);

    return padded;
  }

  /** Pad every track out to the same length, so the mix lines up at the end. */
  async padAllTracksTo(targetLocalOffsetMs) {
    for (const track of this.tracks.values()) {
      await this.padTrackTo(track, targetLocalOffsetMs);
    }
  }
   ...
   */
  async onSpeakingStart(discordUserId) {
    ...
    // Subscribe before writing the silence, so nothing is missed while it
    // drains.  The receive stream buffers in the meantime, and opus frames are
    // ~100 bytes at 50/second, so that is a few KB rather than a problem.
    ...
    // Awaited, so a long gap is written under backpressure; nothing is piped
    // into the track until its silence is on disk.
    await this.padTrackTo(track, startedLocalMs);

    // The meeting may have been stopped — or paused — while that drained.  A
    // pause has to actually stop audio reaching disk, not merely stop new
    // subscriptions, because that is what the announcement embed promised.
    ...
    decoder.on("data", (chunk) => {
      // stop() ends these streams; a late frame arriving afterwards would be a
      // write-after-end, and it has nowhere useful to go in any case.
  ...
  async stop({ reason = "stopped" } = {}) {
    ...
    await this.padAllTracksTo(durationLocalMs);
...
export async function startRecording({ client, sessionId, eventId = null, voiceChannel, actorId = null }) {
  ...
  recorder.connection.receiver.speaking.on("start", (discordUserId) => {
    // Async, so its rejection needs somewhere to land: one speaker's burst
    // failing must not take down the recording for everyone else.
    recorder.onSpeakingStart(discordUserId).catch((error) =>
      console.error(
        `[meetingRecorder] capture failed for ${discordUserId}:`,
        error?.message ?? error
      )
    );
  });

...
/**
 * Attach local settings to the live Stripe item list.
 *
 * The /webstore storefront builds itself from Stripe, so there is no local row
 * per product -- webstoreItemSettings supplies the category and the visibility
 * flag, keyed by Stripe price ID. A price with no settings row keeps the
 * behaviour it had before the table existed: visible, uncategorised.
 *
 * Returns items shaped the way groupByCategory expects, so the storefront can
 * group and filter them exactly like the rank catalog.
 *
 * @param {Array<object>} items       from getWebstoreItems()
 * @param {Array<object>} settings    webstoreItemSettings rows
 * @param {Array<object>} categories  webstoreCategories rows
 */
export function applyItemSettings(items, settings, categories) {
  const settingsByPrice = new Map(
    (Array.isArray(settings) ? settings : [])
      .filter(Boolean)
      .map((s) => [String(s.stripePriceId), s])
  );
  ...
  return (Array.isArray(items) ? items : []).filter(Boolean).map((item) => {
    const setting = settingsByPrice.get(String(item.stripePriceId));
    const category = setting && setting.categoryId != null
      ? categoryById.get(setting.categoryId)
      : null;

    return {
      ...item,
      visible: setting ? flagIsSet(setting.visible) : true,
      categoryId: category ? category.id : null,
      categoryName: category ? category.name : null,
      categoryVisible: category ? flagIsSet(category.visible) : true,
      categorySortOrder: category ? Number(category.sortOrder) || 0 : 0,
      // A local order wins; otherwise fall back to the Stripe metadata order
      // the storefront already sorted by.
      sortOrder: setting && setting.sortOrder != null
        ? Number(setting.sortOrder) || 0

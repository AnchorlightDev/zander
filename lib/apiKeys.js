...
const SCOPE_ROUTES = [
  ...
  // configApiRoute is registered twice: here (token-protected) and again in
  // app.js under the /api/config prefix with no token check. Only these two
  // copies are gated — /api/config/* is deliberately public and is what the
  // zander-addon plugin calls, so it is intentionally absent from this map.
  ["/policy", "config"],
  ["/social", "config"],

...
export function organizationNode(config) {
  ...
  return prune({
    ...
    // Only emitted when an operator has actually said where they are -- an
    // invented country is worse than a missing one.
    address: region.countryCode
      ? { "@type": "PostalAddress", addressCountry: region.countryCode }
      : undefined,
    areaServed: region.name || undefined,
    sameAs,
  });
}

 ...
 */
export function buildLockCopy(requiredRanks, rankMeta, supporter, isLoggedIn) {
  const names = normaliseRankSlugs(requiredRanks).map(
    (slug) => rankMeta?.get?.(slug)?.displayName || slug
  );
  ...
  if (!isLoggedIn) {
    return {
      ...
      body: `This event is for ${rankList} members. Sign in to check your access.`,
      ctaLabel: "Sign In",
      ctaUrl: "/login",
      supporter,
      rankList,
  ...
  if (supporter) {
    return {
      heading: "Supporter event",
      body: `The full details of this event are for ${rankList} members. Support the server to unlock it — and everything else that comes with the rank.`,
  ...
  return {
    heading: "Restricted event",
    body: `The full details of this event are only visible to ${rankList} members.`,
    ctaLabel: null,
    ctaUrl: null,
    supporter,
    rankList,
